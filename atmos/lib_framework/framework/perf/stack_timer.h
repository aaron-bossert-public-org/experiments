...
#	define STACK_TIMER_THREAD( name )                   \
		static thread_callstack_tracker* ___tracker___ = \
			stack_timer::aquire_tracker( name );         \
		stack_timer::begin_thread( ___tracker___ );

#	define STACK_TIMER( name ) STACK_TIMER_ID( __COUNTER__, name, "" )

#	define STACK_TIMER_PAIR( name, instance_desc ) \
		STACK_TIMER_ID( __COUNTER__, name, instance_desc )

#	define STACK_TIMER_START( token )                \
		static size_t ___##token##callsite_index___ = \
			stack_timer::make_callsite( #token );     \
		stack_timer ___##token##timer___( ___##token##callsite_index___ );

#	define STACK_TIMER_END( token ) ___##token##timer___.end();
...
// provides measurement of time spent in a given scope, and inserts results into
// unique per thread tracker
class stack_timer
{
...
public:
	...
	// thread specific: in debug builds implementation asserts this callsite is
	// becomes active only on the thread it was made
...
#	define STACK_TIMER_ID( counter, name, instance_desc )                 \
		static size_t STACK_TIMER_SMASH( ___callsite_index___, counter ) = \
			stack_timer::make_callsite( name );                            \
		stack_timer STACK_TIMER_SMASH( ___scope_timer___, counter )(       \
			STACK_TIMER_SMASH( ___callsite_index___, counter ),            \
			instance_desc );
