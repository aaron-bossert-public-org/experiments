...
namespace concurrent
{
	...
	{
	public:
		write_lock_ptr( T* ptr, lock lk ) : lock_ptr< T > { ptr }, _lk { std::move( lk ) }
		{}

		write_lock_ptr( write_lock_ptr&& other ) : lock_ptr< T > { std::move( other ) }, _lk { std::move( other._lk ) }
		{}
	...
	{
	public:
		read_lock_ptr( const T* ptr, shared_lock lk ) : lock_ptr< const T > { ptr }, _lk { std::move( lk ) }
		{}
	...
	template < typename T >
	typename lockable_individual< T >::write_lock_ptr lockable_individual< T >::write_lock()
	{
		return write_lock_ptr { &_t, lock { _mtx } };
	}

	template < typename T >
	typename lockable_individual< T >::write_lock_ptr lockable_individual< T >::try_write_lock()
	{
		lock lk { _mtx, std::try_to_lock };
		// auto lk = std::unique_lock<std::shared_mutex>(_mtx, std::try_to_lock);
	...
	template < typename T >
	typename lockable_individual< T >::read_lock_ptr lockable_individual< T >::read_lock() const
	{
		return read_lock_ptr { &_t, shared_lock { _mtx } };
	}

	template < typename T >
	typename lockable_individual< T >::read_lock_ptr lockable_individual< T >::try_read_lock() const
	{
