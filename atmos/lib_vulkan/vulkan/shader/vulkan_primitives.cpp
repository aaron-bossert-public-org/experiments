...
namespace
{
	vulkan_primitive::vulkan_variant_t to_vulkan_variant(const std::shared_ptr<compute_buffer> ptr)
	{
		return std::dynamic_pointer_cast<vulkan_compute_buffer, compute_buffer>(ptr);
	}

	vulkan_primitive::vulkan_variant_t to_vulkan_variant(const std::shared_ptr<texture2d> ptr)
	{
		return std::dynamic_pointer_cast<vulkan_texture2d, texture2d>(ptr);
	}

	vulkan_primitive::vulkan_variant_t to_vulkan_variant(const std::shared_ptr<depth_texture2d> ptr)
	{
		return std::dynamic_pointer_cast<vulkan_depth_texture2d, depth_texture2d>(ptr);
	}

	vulkan_primitive::vulkan_variant_t to_vulkan_variant(const std::shared_ptr<render_texture2d> ptr)
	{
		return std::dynamic_pointer_cast<vulkan_render_texture2d, render_texture2d>(ptr);
	}
}
...
std::unique_ptr<vulkan_primitives> vulkan_primitives::make(const config& cfg)
{
	...
	for (const auto& prim_cfg : cfg)
	{
		std::visit(
			[&](auto&& ptr) {
				vulkan_primitive::config vulkan_cfg = { prim_cfg, to_vulkan_variant(ptr) };
