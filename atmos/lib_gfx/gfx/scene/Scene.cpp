...
using namespace gfx
	...
	void
	scene::render( float delta_seconds )
{
	...
	if ( !scene_is_empty )
	{
		...
		_scene_parameter_manager->time( std::fmodf(
			_scene_parameter_manager->time() + delta_seconds,
			60.0f * 60.0f * 2.0f ) );
		...
		_scene_parameter_manager->view_proj_matrix(
			_view_state->view_proj_matrix() );
...
const std::shared_ptr< scene_parameters_manager >& scene::
	scene_parameter_manager() const
{
