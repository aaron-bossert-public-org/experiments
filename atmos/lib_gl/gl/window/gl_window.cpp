	...
	on_resize_t on_resize )
{
	...
	auto glfw_window =
		glfwCreateWindow( res.x, res.y, cfg.name.c_str(), nullptr, nullptr );

	auto w = std::unique_ptr< gl_window >(
		new gl_window( cfg, glfw_window, std::move( on_resize ) ) );
	...
	if ( w->_on_resize )
	{
		glfwSetFramebufferSizeCallback(
			...
			[]( GLFWwindow* glfw_window, int /* width */, int /* height */ ) {
				gl_window* w =
					(gl_window*)glfwGetWindowUserPointer( glfw_window );
				w->_on_resize( w );
			} );
	}
