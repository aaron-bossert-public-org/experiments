...
inline buffer_view_base::buffer_view_base(
	size_t size,
	void* data,
	size_t stride )
	: _size( size )
...
template < typename T >
inline buffer_view_base::buffer_view_base( const T& ary_view )
	: _size( ary_view.size() )
	, _stride( sizeof( T::value_type ) )
	, _data( (void*)ary_view.data() )
{}

inline void* buffer_view_base::data() const
{
	return _data;
}

inline void buffer_view_base::reset()
{
	_size = 0;
	_data = nullptr;
}

inline size_t buffer_view_base::size() const
{
	return _size;
}

inline size_t buffer_view_base::byte_size() const
{
	return _size * _stride;
}

inline size_t buffer_view_base::stride() const
{
	return _stride;
}

inline void* buffer_view_base::at( size_t i ) const
{
	if ( i < _size )
	{
		return _at( i );
...
inline bool buffer_view_base::valid() const
{
	return _size && _stride && _data;
}

inline void* buffer_view_base::_at( size_t i ) const
{
...
template < typename T >
const T* buffer_view< T >::data() const
{
	return (const T*)_data;
}

template < typename T >
T* buffer_view< T >::data()
{
	return (T*)_data;
}
...
template < typename T >
size_t buffer_view< T >::iter::operator-(
	const typename buffer_view< T >::iter& other ) const
{
#if ATMOS_BUFFER_VIEW_BOUNDS_CHECK
