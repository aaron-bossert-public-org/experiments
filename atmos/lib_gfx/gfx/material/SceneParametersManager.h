...
namespace gfx
{
	...
	class scene_parameters_manager
		: public std::enable_shared_from_this< scene_parameters_manager >
	{
	public:
		using scene_program_binding_map_t = std::unordered_map<
			igpu::program*,
			std::weak_ptr< scene_program_binding > >;

		void set_scene_parameters( const std::shared_ptr< parameter_block >& );


#define GETTERS( param, type ) \
	type const& param()        \
	{                          \
		return _##param;       \
	}
#define SETTERS( param, type )    \
	void param( const type& val ) \
	{                             \
		_##param = val;           \
		set( #param, val );       \
	}
