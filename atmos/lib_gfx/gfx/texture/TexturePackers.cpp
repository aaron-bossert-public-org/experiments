...
void texture_packers::process_requests( requests_t& requests )
{
	...
	for ( auto& pair : map )
	{
		...
		if ( find == _packers.end() )
		{
			...
			{
				// texture packer not found for given format, load the texel
				// buffer into a non packed texture.
				LOG_CONTEXT(
					INFORM,
					"could not find packer for format %s, texture:%s",
					igpu::to_string( format ).data(),
					request.raw().msg().name.c_str() );
				request.packed()->process(
					igpu::texture_2d::make( request.raw() ),
					glm::vec2( 1, 1 ),
					glm::vec2( 0, 0 ) );
			}
		}
...
std::unique_ptr< texture_packers > texture_packers::make()
{
	...
	texture_packer::msg::flags pvr_yx = texture_packer::msg::flags(
		texture_packer::msg::flags::MORTON |
		texture_packer::msg::flags::TRANSPOSED );
	...
	return std::unique_ptr< texture_packers >(
		new texture_packers( std::move( packers ) ) );
}
