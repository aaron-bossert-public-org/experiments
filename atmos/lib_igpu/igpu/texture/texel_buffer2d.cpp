...
std::unique_ptr<texel_buffer2d> load_as_stb(
	const std::string& name,
	const sampler& sampler,
	buffer_view<char> raw_file_data);

std::unique_ptr<texel_buffer2d> load_as_ktx(
	const std::string& name,
	const sampler& sampler,
	buffer_view<char> raw_file_data);

std::unique_ptr<texel_buffer2d> load_as_dds(
	const std::string& name,
	const sampler& sampler,
	buffer_view<char> raw_file_data);

std::unique_ptr<texel_buffer2d> load_as_pvr(
	const std::string& name,
	const sampler& sampler,
	buffer_view<char> raw_file_data);

std::unique_ptr<const texel_buffer2d> texel_buffer2d::make(
	const std::string& name,
	const sampler& sampler,
	buffer_view<char> raw_file_data)
{
	std::unique_ptr<const texel_buffer2d> texel_buffer2d;

	(texel_buffer2d = load_as_stb(name, sampler, raw_file_data)) ||
	(texel_buffer2d = load_as_dds(name, sampler, raw_file_data)) ||
	(texel_buffer2d = load_as_pvr(name, sampler, raw_file_data));
...
const texel_buffer2d::config& texel_buffer2d::cfg() const
{
    return _cfg;
}

const buffer_view<char>& texel_buffer2d::texture_data() const
{
    return _texture_data;
}

texel_buffer2d::texel_buffer2d(
	const config& cfg,
	const buffer_view<char>& texture_data,
	std::shared_ptr<const char[]> intermediate_data)
	: _cfg(cfg)
...
std::unique_ptr<texel_buffer2d> load_as_stb(
	...
	buffer_view<char> raw_file_data)
{
	// if this is STBI_default, stb_image will leave the color channel count as it is stored in the image format.
	constexpr int req_comp = STBI_default;

	// if this is 16, stb_image will leave bit depth as it is stored in the image format.
	constexpr int max_bits_per_channel = 16;

	stbi__context s;
	stbi__result_info ri;
	stbi__start_mem(&s, (const stbi_uc*)raw_file_data.data(), (int)raw_file_data.size());
	void* result = nullptr;
	int comp = 0;
	texel_buffer2d::config cfg = {
		name,
		sampler,
	};
	...
	if (stbi__hdr_test(&s))
	{
		result = stbi__hdr_load(&s, &cfg.res.x, &cfg.res.y, &comp, req_comp, &ri);
	}
	else
	{
		result = stbi__load_main(&s, &cfg.res.x, &cfg.res.y, &comp, req_comp, &ri, max_bits_per_channel);
	}

	if (result)
	{
		size_t size = (cfg.res.x * cfg.res.y * comp * ri.bits_per_channel) / 8;
		buffer_view<char> texture_data = buffer_view<char>(size, (char*)result);
		auto intermediate_data = std::shared_ptr<const char[]>((const char*)result, [](const char* result) {
		...
		if (ri.bits_per_channel == 8)
		{
			...
			{
			case STBI_grey:
				cfg.format = texture_format::INT_R8;
				break;
			case STBI_grey_alpha:
				cfg.format = texture_format::INT_R8G8;
				break;
			case STBI_rgb:
				cfg.format = texture_format::INT_R8G8B8;
				break;
			case STBI_rgb_alpha:
		...
		if (ri.bits_per_channel == 16)
		{
			...
			{
			case STBI_grey:
				cfg.format = texture_format::INT_R16;
				break;
			case STBI_grey_alpha:
				cfg.format = texture_format::INT_R16G16;
				break;
			case STBI_rgb:
				cfg.format = texture_format::INT_R16G16B16;
				break;
			case STBI_rgb_alpha:
		...
		return std::unique_ptr<texel_buffer2d>(
			new texel_buffer2d(
				cfg,
				texture_data,
	...
	buffer_view<char> raw_file_data)
{
	...
	constexpr uint8_t kKTXidentifier[12] = { 0xAB, 0x4B, 0x54, 0x58, 0x20, 0x31, 0x31, 0xBB, 0x0D, 0x0A, 0x1A, 0x0A };
	KTXFileHeader* ktxHeader = (KTXFileHeader*)raw_file_data.data();
	KTXDataBlock* ktxData = (KTXDataBlock*)(sizeof(KTXFileHeader) + ktxHeader->bytesOfKeyValueData + (char*)ktxHeader);

	if (raw_file_data.size() < sizeof(KTXFileHeader) ||
		!memcmp(ktxHeader->identifier, kKTXidentifier, sizeof(kKTXidentifier)))
	{
		return nullptr;
	}
	...
	buffer_view<char> texture_data = buffer_view<char>(ktxData->imageSize, ktxData->pixels);	
	...
	buffer_view<char> raw_file_data)
{
	...
	if (raw_file_data.size() < (sizeof(DDS_MAGIC) + sizeof(DDS_HEADER)) ||
		dwMagicNumber != DDS_MAGIC)
	{
		return nullptr;
	}
	
	auto ddsHeader = reinterpret_cast<const DDS_HEADER*>((char*)raw_file_data.data() + sizeof(uint32_t));
	...
	if (ddsHeader->ddspf.flags & DDSF_FOURCC)
	{
		...
		auto texture_data = buffer_view<char>(
		...
		case makeFourCC('D', 'X', 'T', '1'):
			cfg.format = texture_format::DXT_1_RGBA;
			break;
		case makeFourCC('D', 'X', 'T', '3'):
			cfg.format = texture_format::DXT_3_RGBA;
			break;
...
#include <pvr/tools/PVRTTexture.h>
#pragma warning(pop)
	...
	buffer_view<char> raw_file_data)
{
	...
	else if (raw_file_data.size() > sizeof(PVR_Texture_Header) &&
	...
	if(texture_data.size())
	{
		...
		case ePVRTPF_PVRTCI_2bpp_RGB:
			cfg.format = texture_format::PVR1_2BPP_RGB;
			break;
		case ePVRTPF_PVRTCI_2bpp_RGBA:
			cfg.format = texture_format::PVR1_2BPP_RGBA;
			break;
		case  ePVRTPF_PVRTCI_4bpp_RGB:
			cfg.format = texture_format::PVR1_4BPP_RGB;
			break;
		case  ePVRTPF_PVRTCI_4bpp_RGBA:
			cfg.format = texture_format::PVR1_4BPP_RGBA;
			break;
		case  ePVRTPF_PVRTCII_2bpp:
			cfg.format = texture_format::PVR2_2BPP;
			break;
		case  ePVRTPF_PVRTCII_4bpp:
			cfg.format = texture_format::PVR2_4BPP;
			break;
		case ePVRTPF_ETC1:
			cfg.format = texture_format::ETC1_RGB;
			break;
		}
