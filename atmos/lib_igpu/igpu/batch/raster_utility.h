...
namespace igpu
{
	...
	namespace raster_utility
	{
		...
		template < typename RASTER_BATCH_ROOT, typename RASTER_STATE >
		void raster_opaque(
			RASTER_BATCH_ROOT& raster_batch_root,
			RASTER_STATE& raster_state )
		{
			for ( auto& program_batch : raster_batch_root )
			{
			...
			if ( raster_state.batches.root )
			{
				...
				raster_batch_root.stop_raster();
			}
		}

		template < typename BASE_T, typename ITEM_T, typename CHILD_T >
		class raster_impl_t : public BASE_T
		{
		public:
			...
			~raster_impl_t() override = default;

			raster_impl_t() = default;
			raster_impl_t( raster_impl_t&& ) = default;
			raster_impl_t& operator=( raster_impl_t&& ) = default;

		private:
			map_t _map;
		};

		template < typename RASTER_BATCH_ROOT >
		class raster_binding_t : public raster_binding
		{
		public:
			using raster_batch_root_t = typename RASTER_BATCH_ROOT;
			using program_batch_t = typename raster_batch_root_t::child_t;
			...
			using program_ptr_t = bptr_t< raster_batch_root_t >;
			...
			raster_binding_t(
				raster_batch_root_t* raster_batch_root,
				...
				const config_t& cfg,
				ARGS&&... args )
				: _raster_batch_root( raster_batch_root )
				, _program_ptr( raster_batch_root->ptr( program_batch ) )
			...
			~raster_binding_t() override
			{
				...
				auto& map = _raster_batch_root->map();
		...
		private:
			raster_batch_root_t* _raster_batch_root;
