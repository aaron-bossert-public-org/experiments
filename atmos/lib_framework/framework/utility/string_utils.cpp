...
void string_utils::format_with_va_args(
	std::string* result,
	const char* fmt,
...
std::string string_utils::format_with_va_args(
	const char* fmt,
	const std::va_list& args )
{
	std::string result;
...
void string_utils::split_single_delim(
	std::vector< std::string >* result,
	const_ref in,
...
std::vector< std::string > string_utils::split_single_delim(
	const_ref in,
	const char delimiter )
{
	std::vector< std::string > out;
...
void string_utils::split_string_delim(
	std::vector< std::string >* result,
	const_ref in,
	const_ref delimiter )
{
	...
	while ( start < length )
	{
		while ( i < length )
		{
			if ( in[i] == delimiter[0] )
			{
				...
				for ( delimit_check_index = 0;
					  ( delimit_check_index < (int)delimiter.length() ) &&
					  ( i + delimit_check_index < (int)in.length() );
					  ++delimit_check_index )
				{
					if ( in[i + delimit_check_index] !=
						 delimiter[delimit_check_index] )
						break;
				}
...
std::vector< std::string > string_utils::split_with_escaped_quotes(
	const_ref in,
	const_ref delim )
{
