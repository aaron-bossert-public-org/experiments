...
namespace
{
	...
	double get_time_now()
	{
		...
		return std::chrono::
			duration< double, std::chrono::milliseconds::period >( dt )
				.count();
	}
}
...
public:
	callsite_data(
		...
		thread_callstack_tracker& thread_callstack_tracker )
		: _name( name )
...
#	endif
		...
		if ( is_outputting_performance_results )
		{
			...
			// prevent output collisions between separate
			// thread_callstack_trackers
			...
			{
				LOG_DEBUG(
					"||||||||||||||||||||| long frame trap sprung!!! - \n" );
			}

			LOG_DEBUG(
				" %.2f - frame time (milliseconds)\n\n",
	...
	void output_results( double frame_time_milliseconds ) const
	{
		...
		for ( const auto& stack_entry : _stack_entries )
		{
			...
			if ( s_minimum_scope_time > stack_entry.tot_runtime() )
				continue;

			if ( 0 == stack_depth )
				untracked_frame_time_milliseconds -= stack_entry.tot_runtime();
		...
		LOG_DEBUG( "%s\n", get_indent( 0 ) );
		LOG_DEBUG(
			"%s%.2f / %.2f : %s untracked time \n\n\n",
			get_indent( 0 ),
			untracked_frame_time_milliseconds,
			frame_time_milliseconds,
			thread_name );
	}

	size_t begin_callsite(
		size_t callsite_index,
		const std::string& instance_desc )
	{
		ASSERT_CONTEXT( callsite_index < s_callsites.size() );

		// if not hitting the same callsite_index as the last time at this call
		// stack depth, clear records of the last callsites below this point in
		// the stack, and make a new stack entry
		if ( instance_desc.size() ||
			 _stack_depth >= _last_entry_for_stack_depth.size() ||
			 _stack_entries[_last_entry_for_stack_depth[_stack_depth]]
					 .callsite_index() != callsite_index )
		{
			...
			_stack_entries.push_back(
				stack_entry( callsite_index, _stack_depth, instance_desc ) );
		}
	...
	void end_callsite( size_t stack_entry_index )
	{
		_stack_depth--;
		ASSERT_CONTEXT(
			_last_entry_for_stack_depth[_stack_depth] == stack_entry_index );
		ASSERT_CONTEXT(
			_stack_entries[_last_entry_for_stack_depth[_stack_depth]]
	...
	static thread_callstack_tracker* aquire_tracker()
	{
		...
		LOG_CRITICAL(
			"ran out of thread callstack trackers, consider increasing the "
			"limit, or using another call timing mechanism." );
...
{
	thread_callstack_tracker* tracker =
		thread_callstack_tracker::aquire_tracker();
...
{
	if ( thread_callstack_tracker* tracker =
			 thread_callstack_tracker::aquire_tracker() )
	{
...
{
	thread_callstack_tracker::aquire_tracker()
		->next_frame_after_this_callstack();
}
