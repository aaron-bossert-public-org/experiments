...
#	define ASSERT_CAST(T, VAL) [&] (const char* func_sig) {\
auto val = VAL; \
if (false == logging::assert_context(__FILE__, __LINE__, func_sig, "", !val || dynamic_cast<T>(val), "%s is not expected type: %s", #VAL, #T))\
{ __debugbreak(); } \
return reinterpret_cast<T>(val); }(__FUNCSIG__)

#else
#	define ASSERT_CONTEXT(...) ASSERT_CONTEXT_UNUSED_ARGS_(__VA_ARGS__)
#	define ASSERT_CONTEXT_UNUSED_ARGS_(...) VA_DISTRIBUTE_OP(ASSERT_CONTEXT_UNUSED_ARG_, __VA_ARGS__)
