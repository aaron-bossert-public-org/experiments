...
void parse_spirv_resources(Compiler &compiler, igpu::shader_stages stage, ShaderResources &resources,
                           std::array<std::array<uint8_t, 64>, 3> *out_indices, std::vector<parameter> *out_parameters)
{
	...
	for (const auto &resource_category : resource_categories)
	{
		for (auto &resource : *resource_category.p_resources)
		{
			...
			if (spir_type.basetype != SPIRType::Struct && spir_type.basetype != SPIRType::SampledImage)
			{
				LOG_CRITICAL("%s is %s, not supported as shader uniform", resource.name.c_str(),
			...
			{
				LOG_CRITICAL("multidimensional array resources (eg uniform UBO{} ubo[10][10]) not currently supported");
				continue;
			}
			...
			size_t array_size = spir_type.array.size() ? spir_type.array[0] : 1;
			...
			if (*index == (uint8_t)-1)
			{
				*index = static_cast<uint8_t>(out_parameters->size());
				...
				out_parameters->push_back(
				    { resource.name, resource_category.type, array_size, descriptor_set, binding, stage });
			}
			else
			{
...
#define ERR_CHECK(EXPECT, ACTUAL, FMT, F_OP)                                                         \
	if (EXPECT != ACTUAL)                                                                            \
	{                                                                                                \
		LOG_CRITICAL("descriptor set %d binding %d previously seen as " FMT " but now seen as " FMT, \
		             (int)descriptor_set, (int)binding, F_OP(EXPECT), F_OP(ACTUAL));                 \
	}

				ERR_CHECK(parameter->name, resource.name, "%s", [](const std::string &s) { return s.c_str(); });
				ERR_CHECK(parameter->type, resource_category.type, "%s",
				          [](const igpu::parameter::type &t) { return igpu::parameter::to_string(t).data(); });
...
void parse_vertex_parameters(Compiler &compiler, ShaderResources &resources, std::array<uint8_t, 64> *out_indices,
                             std::vector<vertex_parameter> *out_vertex_parameters)
{
	...
	for (auto &resource : resources.stage_inputs)
	{
		...
		{
			LOG_CRITICAL("%s is %s, not supported as vertex attribute", resource.name.c_str(),
			             spir_type_string(spir_type.basetype));
		...
		{
			LOG_CRITICAL("array vertex attributes (eg attribute vec3[10]) not currently supported");
			continue;
		}
		...
		default:
			LOG_CRITICAL("unhandled component count %d", (int)spir_type.vecsize);
			continue;
		}
		...
		if (*index == (uint8_t)-1)
		{
			*index = static_cast<uint8_t>(out_vertex_parameters->size());
			...
			out_vertex_parameters->push_back({ resource.name, comp, location });
		}
		else
		{
...
#define ERR_CHECK(EXPECT, ACTUAL, FMT, F_OP)                                                               \
	if (EXPECT != ACTUAL)                                                                                  \
	{                                                                                                      \
		LOG_CRITICAL("vertex attribute %d previously seen as " FMT " but now seen as " FMT, (int)location, \
		             F_OP(EXPECT), F_OP(ACTUAL));                                                          \
	}
			ERR_CHECK(vertex_parameter->name, resource.name, "%s", [](const std::string &s) { return s.c_str(); });
			ERR_CHECK(vertex_parameter->components, comp, "%s",
			          [](const igpu::components &c) { return igpu::to_string(c).data(); });
...
void igpu::spirv::parse(std::vector<uint32_t> spirv, std::string entry_point, shader_stages stages,
                        std::vector<parameter> *out_parameters, std::vector<vertex_parameter> *out_vertex_parameters)
{
	...
	for (const auto &point : compiler.get_entry_points_and_stages())
	{
		if (point.name == entry_point)
		{
			...
			if (shader_stages::NONE != (stage & stages))
			{
				...
				parse_spirv_resources(compiler, stage, resources, &parameter_indices, out_parameters);

				if (out_vertex_parameters && stage == shader_stages::VERTEX)
				{
					parse_vertex_parameters(compiler, resources, &vertex_parameter_indices, out_vertex_parameters);
				}
			}
		}
	}
