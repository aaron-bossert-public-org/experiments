...
texture_file_parsing::compressed_parser::compressed_parser(const buffer_view<char>& raw_file_data, int req_component_count)
{
	...
	if (stbi__hdr_test(&s))
	{
		raw = stbi__hdr_load(&s, &res.x, &res.y, &comp, req_component_count, &ri);
	}
	else
	{
		raw = stbi__load_main(&s, &res.x, &res.y, &comp, req_component_count, &ri, max_bits_per_channel);
	}
