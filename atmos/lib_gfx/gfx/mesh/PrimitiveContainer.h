...
namespace gfx
{
	...
	public:
		...
		primitive_container& combine(
			const prim_container& primitive_container );
	...
	template < typename vertex, typename index >
	template < typename generator >
	primitive_container< vertex, index >::primitive_container(
	...
	template < typename vertex, typename index >
	template < typename generator >
	primitive_container< vertex, index >& primitive_container< vertex, index >::
		operator=( const generator& generator )
	{
	...
	template < typename prim_container >
	primitive_container< vertex, index >& primitive_container< vertex, index >::
		combine( const prim_container& other )
	{
		...
		std::transform(
			...
			other.vertices().end(),
			_vertices.begin() + vertex_start,
			[]( const typename prim_container::vertex_t& vertex ) {
				vertex cast_vertex;
		...
		std::transform(
			other.indices().begin(),
			other.indices().end(),
			_indices.begin() + index_start,
			[vertex_start]( const typename prim_container::index_t index ) {
	...
	template < typename vertex, typename index >
	primitive_container< vertex, index >& primitive_container< vertex, index >::
		combine( const primitive_container& other )
	{
		unsigned vertex_start = _vertices.size();
		unsigned index_start = _indices.size();
		...
		std::transform(
			...
			other.indices().end(),
			_indices.begin() + index_start,
			[vertex_start]( const index_t index ) {
				return index + vertex_start;
			} );
	...
	template < typename vertex, typename index >
	const std::vector< index >& primitive_container< vertex, index >::indices()
		const
	{
		return _indices;
	}
	...
	template < typename vertex, typename index >
	const std::vector< vertex >& primitive_container< vertex, index >::
		vertices() const
	{
		return _vertices;
	}
