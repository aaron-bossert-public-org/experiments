...
std::unique_ptr< model_geometry > model_geometry::make(
	std::string file,
	mesh_geometries_t mesh_geometries )
{
	...
	material_indices_t material_indices )
{
	...
	return std::unique_ptr< model_geometry >( new model_geometry(
		std::move( file ),
		std::move( mesh_geometries ),
		std::move( material_indices ) ) );
}
...
const model_geometry::material_indices_t& model_geometry::material_indices()
	const
{
