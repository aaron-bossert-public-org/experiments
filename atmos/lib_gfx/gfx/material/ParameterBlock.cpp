...
parameter_block::parameter_block(
	std::vector< std::pair< parameter_id, igpu::primitive > > config,
	bool slow_hash )
...
parameter_block::parameter_handle parameter_block::lookup(
	const parameter_id& name ) const
{
...
void parameter_block::set(
	const std::vector< std::pair< parameter_id, igpu::primitive > >& config )
{
...
parameter_block::parameter_handle parameter_block::set(
	...
	const igpu::primitive& primitive )
{
	...
	else	// no existing parameter with that name, add a new one
	{
		...
		// add parameter to _parameter_bindings, notify block that parameters
		// need rebinding
...
void parameter_block::set(
	const parameter_handle& handle,
	const igpu::primitive& primitive_ )
{
	if ( handle != -1 )
	{
		...
		primitive.hash(
			_slow_hash ? igpu::primitive::make_hash( primitive ) : _hash );
	}
}
...
const igpu::primitive& parameter_block::get(
	const parameter_handle& handle ) const
{
	if ( -1 != handle )
	{
