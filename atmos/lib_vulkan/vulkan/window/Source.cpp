...
namespace vks
{
	namespace initializers
	{
			...
			uint32_t max_sets)
		{
			...
			descriptorPoolInfo.poolSizeCount = pool_size_count;
			descriptorPoolInfo.pPoolSizes = p_pool_sizes;
			...
			uint32_t descriptor_count)
		{
			...
			descriptorPoolSize.descriptorCount = descriptor_count;
			...
			uint32_t binding_count)
		{
			...
			descriptorSetLayoutCreateInfo.pBindings = p_bindings;
			...
			uint32_t set_layout_count = 1)
		{
			...
			pipelineLayoutCreateInfo.pSetLayouts = p_set_layouts;
			...
			uint32_t descriptor_set_count)
		{
			...
			descriptorSetAllocateInfo.pSetLayouts = p_set_layouts;
		...
		inline VkDescriptorImageInfo descriptor_image_info(VkSampler sampler, VkImageView image_view, VkImageLayout image_layout)
		{
			...
			descriptorImageInfo.imageView = image_view;
			descriptorImageInfo.imageLayout = image_layout;
			...
			VkVertexInputRate input_rate)
		{
			...
			vInputBindDescription.inputRate = input_rate;
			...
			VkBool32 primitive_restart_enable)
		{
			...
			pipelineInputAssemblyStateCreateInfo.primitiveRestartEnable = primitive_restart_enable;
			...
			VkPipelineRasterizationStateCreateFlags flags = 0)
		{
			...
			pipelineRasterizationStateCreateInfo.polygonMode = polygon_mode;
			pipelineRasterizationStateCreateInfo.cullMode = cull_mode;
			pipelineRasterizationStateCreateInfo.frontFace = front_face;
			...
			VkBool32 blend_enable)
		{
			...
			pipelineColorBlendAttachmentState.colorWriteMask = color_write_mask;
			pipelineColorBlendAttachmentState.blendEnable = blend_enable;
			...
			const VkPipelineColorBlendAttachmentState * p_attachments)
		{
			...
			pipelineColorBlendStateCreateInfo.attachmentCount = attachment_count;
			pipelineColorBlendStateCreateInfo.pAttachments = p_attachments;
			...
			VkCompareOp depth_compare_op)
		{
			...
			pipelineDepthStencilStateCreateInfo.front = pipeline_depth_stencil_state_create_info.back;
			...
			VkPipelineViewportStateCreateFlags flags = 0)
		{
			...
			pipelineViewportStateCreateInfo.viewportCount = viewport_count;
			pipelineViewportStateCreateInfo.scissorCount = scissor_count;
			...
			VkPipelineMultisampleStateCreateFlags flags = 0)
		{
			...
			pipelineMultisampleStateCreateInfo.rasterizationSamples = rasterization_samples;
			...
			VkPipelineDynamicStateCreateFlags flags = 0)
		{
			...
			pipelineDynamicStateCreateInfo.pDynamicStates = p_dynamic_states;
			...
			VkPipelineDynamicStateCreateFlags flags = 0)
		{
			VkPipelineDynamicStateCreateInfo pipeline_dynamic_state_create_info{};
			pipelineDynamicStateCreateInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_DYNAMIC_STATE_CREATE_INFO;
			pipelineDynamicStateCreateInfo.pDynamicStates = p_dynamic_states.data();
		...
		inline VkPipelineTessellationStateCreateInfo pipeline_tessellation_state_create_info(uint32_t patch_control_points)
		{
			...
			pipelineTessellationStateCreateInfo.patchControlPoints = patch_control_points;
			...
			uint32_t offset)
		{
			...
			pushConstantRange.stageFlags = stage_flags;
		...
		inline VkSpecializationMapEntry specialization_map_entry(uint32_t constant_i_d, uint_32_t offset, size_t size)
		{
			VkSpecializationMapEntry specialization_map_entry{};
			specializationMapEntry.constantID = constant_i_d;
		...
		inline VkSpecializationInfo specialization_info(uint32_t map_entry_count, const VkSpecializationMapEntry* map_entries, size_t data_size, const void* data)
		{
			...
			specializationInfo.mapEntryCount = map_entry_count;
			specializationInfo.pMapEntries = map_entries;
			specializationInfo.dataSize = data_size;
...
#undef STR
		...
		VkPipelineStageFlags dst_stage_mask = VK_PIPELINE_STAGE_ALL_COMMANDS_BIT)
	{
		...
		case VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL:
			...
			imageMemoryBarrier.dstAccessMask = image_memory_barrier.dstAccessMask | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;
			break;
		...
		VkPipelineStageFlags dst_stage_mask = VK_PIPELINE_STAGE_ALL_COMMANDS_BIT)
	{
		...
		setImageLayout(cmdbuffer, image, old_image_layout, new_image_layout, subresource_range, src_stage_mask, dst_stage_mask);
	}
	...
	void exit_fatal(std::string message, int_32_t exit_code)
	{
		std::cerr << message << "\n";
		exit(exitCode);
	}
	...
	VkShaderModule load_shader_g_l_s_l(const char *fileName, VkDevice device, VkShaderStageFlagBits stage)
	{
		std::string shader_src = read_text_file(fileName);
		const char *shaderCode = shader_src.c_str();
		...
		memcpy(((uint32_t *)moduleCreateInfo.pCode + 3), shader_code, size + 1);
...
namespace vks
{
	struct VulkanDevice
	{
		...
		*/
		...
		{
			VkCommandBufferAllocateInfo cmd_buf_allocate_info = vks::initializers::commandBufferAllocateInfo(_command_pool, level, 1);
	...
	public:
		...
		*/
			...
			bool force_linear = false)
		{
			...
			gli::texture2d tex_2_d(gli::load(filename.c_str()));
			assert(!tex2D.empty());
			...
			if (useStaging)
			{
				...
				vkCreateImage(device->_logical_device, &imageCreateInfo, nullptr, &_image);
			...
			else
			{
				...
				vkCreateImage(device->_logical_device, &imageCreateInfo, nullptr, &mappableImage);
				...
				tools::setImageLayout(copyCmd, _image, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_UNDEFINED, image_layout);
...
public:
	...
	void build_compute_command_buffer()
	{
		...
		VkBufferMemoryBarrier buffer_barrier = vks::initializers::bufferMemoryBarrier();
	...
	void prepare_storage_buffers()
	{
		...
		vertices._input_state = vks::initializers::pipelineVertexInputStateCreateInfo();
	...
	void setup_descriptor_pool()
	{
		...
		vkCreateDescriptorPool(_device, &descriptorPoolInfo, nullptr, &_descriptor_pool);
	}
	...
	void setup_descriptor_set_layout()
	{
		...
		vkCreatePipelineLayout(_device, &pipelineLayoutCreateInfo, nullptr, &_graphics._pipeline_layout);
	}
	...
	void setup_descriptor_set()
	{
		...
		vkUpdateDescriptorSets(_device, static_cast<uint32_t>(writeDescriptorSets.size()), write_descriptor_sets.data(), 0, NULL);
	}
	...
	void prepare_pipelines()
	{
		...
		pipelineCreateInfo.pVertexInputState = &vertices._input_state;
		...
		vkCreateGraphicsPipelines(_device, _pipeline_cache, 1, &pipelineCreateInfo, nullptr, &_graphics._pipeline);
	}
	...
	void prepare_compute()
	{
		...
		vkCreatePipelineLayout(_device, &pPipelineLayoutCreateInfo, nullptr, &_compute._pipeline_layout);
		...
		// Create pipeline		
		VkComputePipelineCreateInfo compute_pipeline_create_info = vks::initializers::computePipelineCreateInfo(_compute._pipeline_layout, 0);
		computePipelineCreateInfo.stage = load_shader(getAssetPath() + "shaders/computeparticles/particle.comp.spv", VK_SHADER_STAGE_COMPUTE_BIT);
		vkCreateComputePipelines(_device, _pipeline_cache, 1, &computePipelineCreateInfo, nullptr, &_compute._pipeline);
...
{
	VkImageCreateInfo image_c_i{};
	imageCI.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
	imageCI.imageType = VK_IMAGE_TYPE_2D;
	imageCI.format = _depth_format;
	...
	vkCreateImage(_device, &imageCI, nullptr, &_depth_stencil._image);
	VkMemoryRequirements mem_reqs{};
