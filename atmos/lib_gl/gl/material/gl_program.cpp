...
void gl_program::update(size_t batch_drawpass_id, const batch* batch)
{
	...
	for (size_t param_index = 0; param_index < _batch_parameters.size(); ++param_index)
	{
		...
		size_t constraint_index = parameter.constraint_index();
		if (constraint_index < cfg_constraints.parameters.size())
		{
			if (const primitive_block::element* element = batch_parameters.find(constraint_index))
			{
			...
			{
				// use fallback value
				update(parameter, cfg_constraints.parameters[constraint_index].second);
			}
		} 
		else
		{
