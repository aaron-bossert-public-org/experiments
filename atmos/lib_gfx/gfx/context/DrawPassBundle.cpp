		...
		if ( batched )
		{
			if ( _enable )
			{
				...
				size_t scene_stage_index =
					_original_args.technique.scene_stage_index();
				...
				{
					_weak_binding =
						scene_stage->make_binding( _binding_args, scene );
				}
			}
		}
		else
...
void draw_pass::scene( const std::shared_ptr< scene >& scene )
{
	...
	else
	{
		...
		// currently bound, unbind and rebind to hook into the to the new scene
		// stage
...
draw_pass_bundle::draw_pass_bundle(
	const msg& msg,
	std::shared_ptr< model_state > model_state )
...
std::shared_ptr< draw_pass_bundle > draw_pass_bundle::make(
	const msg& msg,
	std::shared_ptr< model_state > model_state )
{
	return std::shared_ptr< draw_pass_bundle >(
		new draw_pass_bundle( msg, std::move( model_state ) ) );
}
...
void draw_pass_bundle::add_pass( const technique& technique )
{
	...
	for ( auto& draw_pass : _draw_passes )
	{
		if ( draw_pass->original_args().technique.scene_stage_index() ==
			 technique.scene_stage_index() )
		{
			LOG_CONTEXT(
				CRITICAL,
				"more than one technique added for stage(%d : %s), you should "
				"call remove_pass for this stage",
	...
	else
	{
		...
		// draw_pass_modifier can be used to alter the way objects are presented
		// to the renderer for diagnostic purposes
		if ( auto draw_pass_modifier = _draw_pass_modifier.lock() )
		{
			draw_pass =
				draw_pass_modifier->make_draw_pass( draw_pass_modifier, msg );
		}
