...
//    GL_TEXTURE_2D, _gl_handle, 0);
//}
//
// std::unique_ptr<gl_render_texture2d> gl_render_texture2d::make(const config&
// cfg)
//{
//	if (0 >= cfg.res.x || 0 >= cfg.res.y)
//	{
//		LOG_CRITICAL("width(%d) and height(%d) must be greater than zero",
// cfg.res.x, cfg.res.y); 		return nullptr;
//	}
//
//	GLenum format;
//	GLenum type;
...
//
//	glTexImage2D(GL_TEXTURE_2D, 0, format, res.x, res.y, 0, format, type,
// nullptr);
//
//	// set texture min/mag filters, wrapping, and debug name
//	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
// to_gl(cfg.sampler.min_filter)); 	glTexParameteri(GL_TEXTURE_2D,
// GL_TEXTURE_MAG_FILTER, to_gl(cfg.sampler.mag_filter));
//	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
// to_gl(cfg.sampler.addressu)); 	glTexParameteri(GL_TEXTURE_2D,
// GL_TEXTURE_WRAP_T, to_gl(cfg.sampler.addressu));
// glLabelObjectEXT(GL_TEXTURE, gl_handle, (GLsizei)cfg.name.size(),
// cfg.name.c_str());
//
//	glBindTexture(GL_TEXTURE_2D, active_handle);
//
//	return std::unique_ptr<gl_render_texture2d>(new gl_render_texture2d(cfg,
// gl_handle));
//}
//
...
//, _gpu_mem_metric(perf::category::GPU_MEM_USAGE, "GPU Render Texture Buffer
// Mem")
//{
