...
namespace
{
	...
	program_instance::bindings_t generate_parameter_bindings(
		...
		const std::shared_ptr< igpu::program >& program )
	{
		...
		// sort program instance parameters loosely based on the cost of
		// updating a program with them

		// for batching, program instances are lexically ordered by their
		// parameter hashes the lexical position of each hash is primarily
		// dependant on cost of updating corrisponding parameter, and
		// secondarily dependent on parameter name
		const auto expensive_params_first = []( const binding_t& lhs,
												const binding_t& rhs ) {
		...
		std::sort(
			...
			parameter_bindings.end(),
			expensive_params_first );

		return std::move( parameter_bindings );
	}

	std::vector< const binding_t* > bound_parameters_for_program(
		const program_instance::lookup_table_t& lookup_table,
...
void program_instance::meld_parameters(
	parameter_block_bindings& mutable_block_bindings,
	const std::shared_ptr< igpu::program >& program )
{
	...
	// swap old and new parameters, the old ones will be reset to values found
	// in the material
	std::swap( _new_block_parameters, _old_block_parameters );


	const auto& parameter_bindings =
		mutable_block_bindings.parameter_bindings( *program );
	const value_hash_t& block_hash =
		mutable_block_bindings.parameter_block().hash();
	...
	if ( _init_program_parameters )
	{
		...
		// reset all parameters to material values iff they were not set by the
		// new parameter block instead of testing hashes for parameer value
		// differences
	...
	{
		// reset old block parameters to material values iff they were not set
		// by the new parameter block.
		for ( auto* old_block_parameter : _old_block_parameters )
		{
			if ( block_hash != old_block_parameter->hash() )
			{
				if ( const auto* binding =
						 _bound_parameter_table[old_block_parameter->index()] )
				{
					program->update(
						old_block_parameter,
						*binding->parameter_binding.primitive(),
						true );
				}
			}
		}
...
const std::shared_ptr< igpu::render_states >& program_instance::render_states()
	const
{
...
bool program_instance::compare_by_parameters(
	const program_instance* lhs,
	const program_instance* rhs )
{
	// dig through the parameters of lhs, returning the comparison result of tht
	// first inequal hashes of equal parameter name.
	...
	for ( size_t p = 0; p < size; ++p )
	{
		...
		// do not assume program instances specify primitive values for the same
		// subesets parameters: compare by parameters by name then the hash of
		// their primitives
...
void program_instance::rebind_parameters(
	const std::shared_ptr< igpu::program >& program ) const
{
	...
	if ( _program_binding_id != program_binding_id )
	{
		...
		_bound_parameter_table =
			bound_parameters_for_program( _bindings, program );
	...
	else
	{
		...
		for ( const auto& handle : parameter_block->handles() )
		{
			...
			if ( !primitive.hash().val() )
			{
				LOG_CONTEXT(
					CRITICAL,
					"parameter %s must have a hashed primitive to be used with "
					"a program instance",
					name.c_str() );
			}
		}
		...
		return std::unique_ptr< program_instance >( new program_instance(
			name,
			types,
			program,
			render_states,
