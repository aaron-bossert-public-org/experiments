...
vulkan_command_buffer::vulkan_command_buffer(const config& cfg, const std::shared_ptr < vulkan_fence >& fence)
...
vulkan_command_buffer::~vulkan_command_buffer()
{
	...
	if(_command_buffer)
	{ 
		vkFreeCommandBuffers(_cfg.device, _cfg.command_pool, 1, &_command_buffer);
	}
}
