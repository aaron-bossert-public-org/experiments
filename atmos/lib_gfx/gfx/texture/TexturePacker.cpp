...
TWEAK_VARIABLE(
	debug_packed_textures,
	false,
	"All packed textures will black with white bordering textures to show "
	"worst case bleed" );

TWEAK_VARIABLE(
	perf_no_packing_uv_twiddling,
	false,
	"If on, packed textures will use their original UVs" );
...
//      - save uv information and a refrence to the atlas inside each packed
//      texture request
...
namespace
{
	...
	{
		// the smallest square pow 2 texture that can hold the given number of
		// texels
	...
	glm::ivec2 deinterleave( unsigned value )
	{
		...
		// this assumes square/powers of 2, and sub textures are sorted in
		// atlases by size
		unsigned x = ( value >> 1 ) & 0X55555555;
		x = ( x | ( x >> 1 ) ) & 0X33333333;
		x = ( x | ( x >> 2 ) ) & 0X0F0F0F0F;
		x = ( x | ( x >> 4 ) ) & 0X00FF00FF;
		x = ( x | ( x >> 8 ) ) & 0X0000FFFF;

		unsigned y = value & 0X55555555;
		y = ( y | ( y >> 1 ) ) & 0X33333333;
		y = ( y | ( y >> 2 ) ) & 0X0F0F0F0F;
		y = ( y | ( y >> 4 ) ) & 0X00FF00FF;
		y = ( y | ( y >> 8 ) ) & 0X0000FFFF;
	...
	void copy_texture_data(
		const std::unique_ptr< char[] >& atlas_backing,
		int atlas_backing_size,
		const glm::ivec2& res,
		const texture_packer::msg& msg,
		const requests_t& requests )
	{
		...
		if ( msg.morton_order )
		{
			// morton order texure formats have implied perfect packing order
			// for sorted square pow2 textures
		...
		{
			// non morton ordered must textures must be copied one block row at
			// a time
			for ( const auto& request : requests )
			{
				const char* texture_data = &request.raw().texture_data()[0];
				glm::ivec2 texture_blocks =
					request.raw().res() / msg.block_resolution;
				glm::ivec2 atlas_blocks = res / msg.block_resolution;
				glm::ivec2 atlas_blocks_offset =
					deinterleave(
						unsigned( bytes_written * msg.bits_per_pixel ) / 8 ) /
					msg.block_resolution;

				int bytes_per_block =
					( msg.block_resolution.x * msg.block_resolution.y *
					  msg.bits_per_pixel ) /
					8;
				int bytes_per_texture_row = texture_blocks.x * bytes_per_block;
				int bytes_per_atlas_row = atlas_blocks.x * bytes_per_block;
				int bytes_offset_atlas =
					atlas_blocks_offset.x * bytes_per_block +
					atlas_blocks_offset.y * bytes_per_atlas_row;

				for ( int block_y = 0; block_y < texture_blocks.y; ++block_y )
				{
					const char* src =
						texture_data + block_y * bytes_per_texture_row;
					char* dst = atlas_data + bytes_offset_atlas +
						block_y * bytes_per_atlas_row;
		...
		const requests_t& requests )
	{
		...
		for ( const packing_request& request : requests )
		{
			...
			const std::shared_ptr< packed_texture > packed_texture =
				request.packed();
			...
			if ( !packed_texture )
			{
				LOG_CONTEXT(
					CRITICAL,
					"unable to process packing info, packed_texture is null "
					"for info(%s)",
					raw.path().c_str() );
			}
			else if ( packed_texture->atlas() )
			{
				LOG_CONTEXT(
					CRITICAL,
					"through weird twisty logic this packed texture has been "
					"processed multiple times (%s)",
					raw.path().c_str() );
			}
			else
			{
				...
				if ( transposed_uv )
					std::swap( uv_offset.x, uv_offset.y );
	...
	void pack_raw_atlas(
		const texture_packer& texture_packer,
		const requests_t& requests,
		const int texel_count )
	{
		if ( texel_count > 0 )
		{
			auto name = " packed " +
			...
			// the most expensive part of creating an atlas is allocating the
			// large buffer required to store the texels 32.93 : pack_raw_atlas
			// ( packed RGB ETC1 #1 diffuse) [2048x2048], 61.91% used, 37
			// textures | 24.90 : make_backing | 1.28 : copy_texture_data | 5.64
			// : upload_atlas | 0.12 : register_atlas_with_requests


			int atlas_backing_size =
				( width * width * texture_packer.msg().bits_per_pixel ) / 8;
			auto atlas_backing =
				std::unique_ptr< char[] >( new char[atlas_backing_size] );
			...
			std::shared_ptr< const igpu::texel_buffer_2d > raw =
				igpu::texel_buffer_2d::make(
					name,
					texture_packer.msg().format,
					res,
					std::move( atlas_backing ),
					atlas_backing_size );

			// make a gpu texture out of packed data
			std::shared_ptr< igpu::texture_2d > atlas =
				igpu::texture_2d::make( *raw );

			// using texture atlas process all the packedrequests in the
			// raw_atlas
			process_packed_info(
				texture_packer.msg().transposed_uv,
				atlas,
				requests );
		}
	}
	...
	void debug_packed_texture_group( const requests_t& requests )
	{
		for ( const auto& request : requests )
		{
			...
			std::fill( data_begin, data_begin + size, (char)0 );

			std::fill( data_begin + size, data_end, (char)-1 );

			auto raw = igpu::texel_buffer_2d::make(
				name,
				request.raw().format(),
				request.raw().res() * factor,
				std::move( data ),
				padded_size );
			...
			request.packed()->process(
				debug_texture,
				glm::ivec2( 1, 1 ) / factor,
				glm::vec2( 0, 0 ) );
		}
	}
}
...
{
	requests.sort(
		[]( const packing_request& lhs, const packing_request& rhs ) {
			return lhs.raw().texture_data().size() >
				rhs.raw().texture_data().size();
		} );
	...
	{
		const size_t max_texel_count =
			_msg.max_atlas_resolution.x * _msg.max_atlas_resolution.y;
		int texel_count = 0;
		...
		while ( at != requests.end() )
		{
			...
			if ( res.x > _msg.max_atlas_resolution.x ||
				 res.y > _msg.max_atlas_resolution.y )
			{
				LOG_CONTEXT(
					CRITICAL,
					"texture[%dx%d](%s) is larger than max allowed "
					"resolution[%dx%d]",
					raw.res().x,
					raw.res().y,
					raw.path().c_str(),
				...
				_msg.morton_order &&
				( ( raw.res().x != raw.res().y ) || !is_pow2( raw.res().x ) ) )
			{
				LOG_CONTEXT(
					CRITICAL,
					"texture[%dx%d](%s) cannot be used for morton order "
					"texture format",
					raw.res().x,
					raw.res().y,
					raw.path().c_str() );
