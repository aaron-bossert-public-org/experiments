...
void scene_parameters_manager::set_scene_parameters(const std::shared_ptr<parameter_block>& parameter_block)
{
    if(!parameter_block)
    {
...
void scene_parameters_manager::_set(const std::string& name, const igpu::primitive& primitive)
{
    ASSERT_CONTEXT(igpu::primitive::TYPE_UNDEFINED != primitive.get_type());
    
    if(!_enable_scene_parameter_updates) return;
...
void scene_parameters_manager::enable_scene_parameter_updates(bool enable_scene_parameter_updates)
{
    if(enable_scene_parameter_updates && !_enable_scene_parameter_updates)
    {
        // update all programs with the current scene parameters after coming back from disabled parameter updates
        for(const auto& pair : _scene_program_binding_map)
        {
            auto scene_program_binding = pair.second.lock();
            ASSERT_CONTEXT(scene_program_binding);
...
const std::unordered_map<std::string, igpu::primitive>& scene_parameters_manager::scene_parameters() const
{
    return _scene_parameters;
}

std::shared_ptr<scene_program_binding> scene_parameters_manager::scene_program_binding(const std::shared_ptr<igpu::program>& program)
{
    ...
    (scene_program_binding::make(weak, program).release(), [weak](scene_program_binding* binding){
        
        if(auto this_ = weak.lock())
        {
            this_->_scene_program_binding_map.erase(binding->program().get());
        }
...
{
    return std::unique_ptr<scene_parameters_manager>(new scene_parameters_manager());
}
