	...
	const int			nVertexBones)
{
	...
	CPVRTBoneBatches::nBatchBoneMax = nBatchBoneMax_;
	pnBatches		= (int*) calloc(lBatch.size() * nBatchBoneMax_, sizeof(*pnBatches));
	...
	for(iBatch = lBatch.begin(); iBatch != lBatch.end(); ++iBatch)
	{
		...
		iBatch->Write(&pnBatches[nBatchCnt * nBatchBoneMax_], &pnBatchBoneCnt[nBatchCnt]);
