...
model_meshes::model_meshes( meshes_t meshes, material_indices_t material_indices )
    ...
    const material_indices_t& material_indices)
{
    ...
    auto meshes = transform< meshes_t >( mesh_geometries, [file]( const mesh_geometry_t& mesh_geometry ) {
        
        return  std::shared_ptr<mesh>(mesh::make(file, mesh_geometry.get()));
    });
    
    return std::unique_ptr<model_meshes>( new model_meshes( std::move( meshes ), material_indices ) );
}
