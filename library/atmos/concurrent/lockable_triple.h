...
namespace concurrent
{
	...
	private:
		...
		mutable std::atomic_int_fast8_t _swap_count = { 0 };
	};
	...
	template<typename T>
	typename lockable_triple<T>::write_lock_ptr lockable_triple<T>::write_lock()
	{
		return write_lock_ptr{ _t + _write_idx, lock(_write_mtx), this };
	}

	template<typename T>
	typename lockable_triple<T>::write_lock_ptr lockable_triple<T>::try_write_lock()
	{
	...
	template<typename T>
	typename lockable_triple<T>::read_lock_ptr lockable_triple<T>::read_lock() const
	{
		lock lk{ _read_mtx };
		read_begin();
	...
	template<typename T>
	typename lockable_triple<T>::read_lock_ptr lockable_triple<T>::try_read_lock() const
	{
	...
	// continually oscillate writing between the two buffers not currently being read from as read buffer changes
	template<typename T>
	...
	void lockable_triple<T>::read_begin() const
	{
		...
		if (!_swap_count.compare_exchange_strong(dummy, _prev_read_swap_count))
		{
			auto swap_count = _swap_count.fetch_add(1);
