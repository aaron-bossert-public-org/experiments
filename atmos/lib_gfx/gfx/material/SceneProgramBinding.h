...
namespace gfx
{
	...
	public:
		...
		static std::unique_ptr< scene_program_binding > make(
			const std::weak_ptr< scene_parameters_manager >&,
			const program_t& );

	private:
		scene_program_binding(
			const std::weak_ptr< scene_parameters_manager >&,
			const program_t& );
	...
	private:
		...
		std::unordered_map< const igpu::parameter*, igpu::primitive >
			_pending_primitives;
