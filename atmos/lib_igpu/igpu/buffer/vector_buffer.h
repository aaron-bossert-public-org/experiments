...
namespace igpu
{
	...
	private:
		...
		config _cfg = { buffer_usage::DYNAMIC };

		buffer_view<T> _view_unmapped = {};
		std::vector<T> _memory;

		perf::metric _cpu_mem_metric = perf::metric(perf::category::CPU_MEM_USAGE, "vector buffer");
	};
	...
	template<typename T>
	vector_buffer<T>& vector_buffer<T>::operator=(vector_buffer<T>&& other) noexcept
	{
		_memory = std::move(other._memory);
		_view_unmapped = other.view_unmapped;
		other.view_unmapped = {};
	...
	template<typename T>
	vector_buffer<T>& vector_buffer<T>::operator=(std::vector<T>&& other) noexcept
	{
		_memory = std::move(other);
		_view_unmapped = other.view_unmapped;
		return *this;
	...
	void vector_buffer<T>::map(buffer_view_base* out_mapped_view)
	{
		...
		{
			size_t count = (out_mapped_view->byte_size() + sizeof(T) - 1) / sizeof(T);
