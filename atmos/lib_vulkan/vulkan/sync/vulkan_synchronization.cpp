...
namespace
{
	...
	const scoped_ptr< vulkan_fence >& submit( vulkan_queue& queue, SubmissonBuilder&& builder )
	{
		...
		queue.pending_commands().emplace_back( cfg, vulkan_fence::make( { queue.cfg().device } ) );
	...
	void submit_sync( const scoped_ptr< vulkan_queue >& queue,
					  CommandBuilder&&					command_builder,
					  Args... args )
	{
		submit_sync( *queue, [&]( VkCommandBuffer command_buffer ) {
			command_builder( command_buffer, add_dependency( command_buffer, queue, args )... );
		} );
	}
	...
	// ensure memory/execution dependencies are added for args
	template < typename CommandBuilder, typename... Args >
	const scoped_ptr< vulkan_fence >& submit( const scoped_ptr< vulkan_queue >& queue,
											  CommandBuilder&&					command_builder,
											  Args... args )
	{
		return submit( *queue, [&]( VkCommandBuffer command_buffer ) {
			command_builder( command_buffer, add_dependency( command_buffer, args.buffer,
															 { args.access, args.stage,
															   args.dependency, queue } )... );
		} );
	}
	...
	image_dependency dependency( vulkan_image&		  image,
								 VkImageLayout		  layout,
								 VkAccessFlags		  access,
								 VkPipelineStageFlags stage,
	...
	VkImage add_dependency( VkCommandBuffer					  command_buffer,
							...
							const image_dependency&			  image_dependency )
	{
		...
		vulkan_image::ownership		   target_owner
			= { image_dependency.layout, image_dependency.access, image_dependency.stage,
				image_dependency.dependency, queue };

		// apparently this is slower than VkMemoryBarrier, need an api to handle these in bulk with
		// VkMemoryBarrier.
		...
		// just a layout transition to initialize image to the specified state, no dependency
		// actually added via vkbarriers this branch
		else if ( !current_owner.queue )
		{
			vkCmdPipelineBarrier( command_buffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
								  target_owner.stage, target_owner.dependency, 0, nullptr, 0,
								  nullptr, 1, &barrier );
		}
		...
		else if ( current_owner.queue == target_owner.queue )
		{
			...
			{
				vkCmdPipelineBarrier( command_buffer, current_owner.stage, target_owner.stage,
									  target_owner.dependency, 0, nullptr, 0, nullptr, 1,
									  &barrier );
			}
		}
		...
		else if ( current_owner.queue )
		{
			...
			submit_sync( current_owner.queue, [&]( VkCommandBuffer current_owner_command_buffer ) {
				vkCmdPipelineBarrier( current_owner_command_buffer, current_owner.stage,
	...
	buffer_dependency dependency( vulkan_buffer&	   buffer,
								  VkAccessFlags		   access,
								  VkPipelineStageFlags stage,
	...
	VkBuffer add_dependency( VkCommandBuffer				   command_buffer,
							 const scoped_ptr< vulkan_queue >& queue,
							 const buffer_dependency&		   buffer_dependency )
	{
		...
		vulkan_buffer::ownership target_owner = { buffer_dependency.access, buffer_dependency.stage,
												  buffer_dependency.dependency, queue };
		...
		else if ( current_owner.queue == target_owner.queue )
		{
			...
			{
				vkCmdPipelineBarrier( command_buffer, current_owner.stage, target_owner.stage,
									  target_owner.dependency, 0, nullptr, 1, &barrier, 0,
									  nullptr );
			}
		}
		...
		else if ( current_owner.queue )
		{
			...
			vkCmdPipelineBarrier( command_buffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
								  target_owner.stage, target_owner.dependency, 0, nullptr, 1,
								  &dest_barrier, 0, nullptr );
		}
...
void vulkan_synchronization::copy( vulkan_buffer& src,
								   vulkan_buffer& dst,
								   uint32_t		  size,
								   uint32_t		  src_offset,
								   uint32_t		  dst_offset )
{
	submit_sync(
		_cfg.transfer_queue,
		[&]( VkCommandBuffer command_buffer, VkBuffer src_buffer, VkBuffer dst_buffer ) {
			VkBufferCopy region = {};
...
void vulkan_synchronization::copy( vulkan_buffer& src, vulkan_image& dst, uint32_t src_offset )
{
	submit_sync(
		_cfg.transfer_queue,
		[&]( VkCommandBuffer command_buffer, VkBuffer src_buffer, VkImage dst_image ) {
		...
		dependency( src, VK_ACCESS_TRANSFER_READ_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT ),
		dependency( dst, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_ACCESS_TRANSFER_WRITE_BIT,
					VK_PIPELINE_STAGE_TRANSFER_BIT ) );
}


bool vulkan_synchronization::can_generate_mipmaps( VkFormat format, VkImageTiling tiling )
{
	VkFlags required = VK_FORMAT_FEATURE_BLIT_SRC_BIT | VK_FORMAT_FEATURE_BLIT_DST_BIT
	...
	switch ( tiling )
	{
		case VK_IMAGE_TILING_OPTIMAL: actual = format_properties.optimalTilingFeatures; break;
		case VK_IMAGE_TILING_LINEAR: actual = format_properties.linearTilingFeatures; break;
	}
...
{
	if ( false
		 == can_generate_mipmaps( image.cfg().image_info.format, image.cfg().image_info.tiling ) )
	{
		LOG_CRITICAL( "vk format %d does not support linear blitting!",
	...
	submit_sync(
		...
		[&]( VkCommandBuffer command_buffer, VkImage vk_image ) {
			...
			// setting by hand, currently there isn't a way to represent per-mip ownership so we do
			// the barriers by hand and track ownership manually. In the future support should be
			// added to track ownership on the mip level
			image.owner( { VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_ACCESS_TRANSFER_READ_BIT,
						   VK_PIPELINE_STAGE_TRANSFER_BIT, 0, _cfg.graphics_queue } );
			...
			for ( uint32_t i = 1; i < image.cfg().view_info.subresourceRange.levelCount; i++ )
			{
				...
				vkCmdBlitImage( command_buffer, vk_image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
								vk_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &blit,
								VK_FILTER_LINEAR );
		...
		dependency( image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_ACCESS_TRANSFER_READ_BIT,
					VK_PIPELINE_STAGE_TRANSFER_BIT ) );
}
...
std::unique_ptr< vulkan_synchronization > vulkan_synchronization::make( const config& cfg )
{
	...
	return std::unique_ptr< vulkan_synchronization >( new vulkan_synchronization( cfg, vma ) );
}

vulkan_synchronization::vulkan_synchronization( const config& cfg, VmaAllocator vma )
	: _cfg( cfg )
	, _vma( vma )
{}
...
private:
	...
	struct new_texture_impl
	{
		...
		void create()
		{
			...
			submit_sync(
				...
				dependency( vulkan->gpu_resource(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
	...
	struct new_geo_impl
	{
		...
		{
			auto index_buffer
				= std::shared_ptr< igpu::index_buffer >( app->_context->make_index_buffer(
					{ buffer_usage::STATIC, index_format::UNSIGNED_INT } ) );

			auto vertex_buffer
				= std::shared_ptr< igpu::vertex_buffer >( app->_context->make_vertex_buffer(
		...
		void draw( VkCommandBuffer command_buffer )
		{
			...
			{
				offsets[i]		  = 0;
				vertex_buffers[i] = vulkan->vertex_buffer( i ).gpu_resource().get();
			}

			vkCmdBindIndexBuffer( command_buffer, index_buffer.gpu_resource().get(), 0,
								  index_buffer.cfg().vk.format );
			vkCmdBindVertexBuffers( command_buffer, 0, 1, vertex_buffers, offsets );
			vkCmdDrawIndexed( command_buffer, (uint32_t)vulkan->index_buffer().element_count(), 1,
							  0, 0, 0 );
		}
	...
	struct new_uniform_impl
	{
		...
		void create()
		{
			...
			{
				_compute_buffers[i]
					= app->_context->make_compute_buffer( { buffer_usage::DYNAMIC } );
				update( i );
			}
		}
...
#pragma warning( pop )
	...
	struct old_program_impl
	{
		void create()
		{
			auto	  vertex_file_data	 = app->_context->make_raw_buffer( {} ),
				 ...
				 app->load_buffer( "cooked_assets/shaders/shader.vert.spv", vertex_file_data );

			app->load_buffer( "cooked_assets/shaders/shader.frag.spv", fragment_file_data );

			program::config program_cfg = {
				"test program",
				app->_context->make_vertex_shader( std::move( vertex_file_data ) ),
				app->_context->make_fragment_shader( std::move( fragment_file_data ) ),
			};

			program				   = app->_context->make_program( program_cfg );
			vulkan_program* vulkan = ASSERT_CAST( vulkan_program*, program.get() );

			shader_stages = { vulkan->cfg().vk.vertex->stage_info(),
	...
	struct new_program_impl
	{
		void create()
		{
			...
			auto* vulkan_fragment
				= ASSERT_CAST( vulkan_fragment_shader*, program_cfg.fragment.get() );

			app->load_buffer( "cooked_assets/shaders/shader.vert.spv", vulkan_vertex );

			app->load_buffer( "cooked_assets/shaders/shader.frag.spv", vulkan_fragment );
	...
	struct new_render_states_impl
	{
		void create()
		{
			...
			_render_states = app->_context->make_render_states( { color_write_mask, c, b, s, d } );
		}
		...
		{
			auto* vulkan = ASSERT_CAST( vulkan_render_states*, _render_states.get() );
		...
		const VkPipelineColorBlendAttachmentState* color_blend_attachment() const
		{
			auto* vulkan = ASSERT_CAST( vulkan_render_states*, _render_states.get() );
	...
	new_render_states_impl render_states_impl = { this };
...
public:
	...
	void cleanup()
	{
		...
		for ( auto* semaphores : { &_render_finished_semaphores, &_image_available_semaphores } )
		{
...
private:
	...
	void create_graphics_pipeline()
	{
		...
		color_blending.pAttachments		 = render_states_impl.color_blend_attachment();
		...
		for ( uint32_t i = 0; i < program->vertex_parameter_count(); ++i )
		{
			...
			{
				VkVertexInputBindingDescription* binding_description = *pp_binding_description
		...
		vkCreateGraphicsPipelines( _device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr,
								   &_graphics_pipeline );
	}
	...
	void load_buffer( const std::string& path, buffer* buffer )
	{
		...
		if ( !ifs )
		{
			throw std::runtime_error(
				EXCEPTION_CONTEXT( "failed to load texture %s", TEXTURE_PATH.c_str() ) );
		}
	...
	{
		std::vector< VkDescriptorSetLayout > layouts( _swap_image_count,
													  program_impl.descriptor_set_layouts[0] );
		...
		for ( size_t i = 0; i < _swap_image_count; i++ )
		{
			...
			vkUpdateDescriptorSets( _device, static_cast< uint32_t >( descriptor_writes.size() ),
	...
	void create_command_buffers()
	{
		...
		for ( size_t i = 0; i < _command_buffers.size(); i++ )
		{
			...
			vkCmdBindDescriptorSets( _command_buffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
									 program_impl.pipeline_layout, 0, 1, &_descriptor_sets[i], 0,
									 nullptr );
	...
	void draw_frame()
	{
		...
		switch ( vkAcquireNextImageKHR(
			_device, _context->back_buffer().swap_chain(), std::numeric_limits< uint64_t >::max(),
			_image_available_semaphores[_current_frame], VK_NULL_HANDLE, &image_index ) )
		{
			case VK_SUBOPTIMAL_KHR:
				...
				LOG_WARNING( "vkAcquireNextImageKHR: VK_SUBOPTIMAL_KHR" );
				break;
			case VK_ERROR_OUT_OF_DATE_KHR: on_recreate_swap_chain(); break;
		}
		...
		if ( vkQueueSubmit( _graphics_queue->get(), 1, &submit_info,
		...
		switch ( vkQueuePresentKHR( _present_queue->get(), &present_info ) )
		{
			case VK_SUBOPTIMAL_KHR:
				LOG_WARNING( "vkAcquireNextImageKHR: VK_SUBOPTIMAL_KHR" );
				[[fallthrough]];
			case VK_ERROR_OUT_OF_DATE_KHR: _framebuffer_resized = true;
		}
