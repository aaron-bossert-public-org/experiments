...
using namespace gfx
	...
	: _model_state(
		  model_state::make( scene, [this]( const glm::mat4x4& model_matrix ) {
...
std::unique_ptr< model_instance > model_instance::make(
	const std::shared_ptr< scene >& scene )
{
	return std::unique_ptr< model_instance >( new model_instance( scene ) );
}

void model_instance::set_model_meshes(
	std::shared_ptr< draw_pass_modifier > draw_pass_modifier,
	...
	technique_sets_t technique_sets )
{
	...
	if ( _model_meshes )
	{
		...
		for ( size_t i = 0; i < meshes.size(); ++i )
		{
			...
			_draw_pass_bundles[i] =
				std::make_shared< draw_pass_bundle >( msg, _model_state );
		}
...
void model_instance::add_draw_passes( const technique_sets_t& technique_sets )
{
	...
	if ( material_indices.size() != _draw_pass_bundles.size() )
	{
		LOG_CONTEXT(
			CRITICAL,
			"material_indices.size(%d) != _draw_pass_bundles.size(%d)",
	...
	for ( size_t i = 0; i < _draw_pass_bundles.size(); ++i )
	{
		...
		if ( material_index >= technique_sets.size() )
		{
			LOG_CONTEXT(
				CRITICAL,
				"material_index(%d) >= technique_sets.size(%d)",
				material_index,
				technique_sets.size() );
			return;
		}
...
void model_instance::remove_draw_passes(
	const technique_sets_t& technique_sets )
{
	auto material_indices = _model_meshes->material_indices();
...
const std::vector< std::shared_ptr< draw_pass_bundle > >& model_instance::
	draw_pass_bundles() const
{
