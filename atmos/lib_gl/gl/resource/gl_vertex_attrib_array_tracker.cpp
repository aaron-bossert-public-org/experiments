...
void gl_vertex_attrib_array_tracker::set_active_vertex_resources(const gl_geometry& geometry)
{
    for(const auto& vertex_resource : geometry.vertex_resources())
    {
		const gl_vertex_resource* gl_v_resource = (const gl_vertex_resource*)vertex_resource.get();
