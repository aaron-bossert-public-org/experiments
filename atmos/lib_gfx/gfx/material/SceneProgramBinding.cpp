...
void scene_program_binding::begin()
{
	...
	if ( _scene_token == _program->scene_token() &&
	...
	else
	{
		...
		if ( scene_parameters_manager )
		{
			// flush pending primitives into program
			for ( const auto& pair :
				  scene_parameters_manager->scene_parameters() )
			{
...
void scene_program_binding::update(
	...
	const program_t& program )
{
	...
	{
		return std::unique_ptr< scene_program_binding >(
			new scene_program_binding( scene_parameters_manager, program ) );
	}

	return nullptr;
}

scene_program_binding::scene_program_binding(
	const std::weak_ptr< scene_parameters_manager >& scene_parameters_manager,
	const program_t& program )
	: _program( program )
